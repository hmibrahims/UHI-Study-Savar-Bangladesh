//2024/25 p75
// =====================================================
// Savar (Dhaka) — SUHI Apr–Sep 2024–2025
// Landsat 8/9 Collection 2 Level 2 (LC08 + LC09 merged)
// UPDATED ROADMAP IMPLEMENTATION:
// - 2-year block (2024–2025), Apr–Sep
...
// -------------------------
// 1) Period settings (2-year block) 2024/2025
// -------------------------
var PERIOD_NAME = '2024_2025_AprSep';
var START_DATE  = '2024-01-01';
var END_DATE    = '2026-01-01';  // end-exclusive
var MONTH_START = 4;
var MONTH_END   = 7;             // Apr–Sep
...
var MAX_WATER_FRAC = 0.30; // if too strict, use 0.30 (keep fixed across years)


// -------------------------
// 3) QA masking (cloud/shadow/saturation) — Landsat 8/9
// -------------------------
function maskL8L9C2(img) {
...
// -------------------------
// 4) LST (ST_B10) to Celsius — Landsat 8/9
// -------------------------
...
// -------------------------
// 6) Scene-level flood filter helper (L8/L9)
// -------------------------
function addWaterFracPerScene_L8L9(img) {
  // L8/9 SR bands: Green=SR_B3, Red=SR_B4, NIR=SR_B5, SWIR1=SR_B6
...
// Landsat 9
var l9raw = ee.ImageCollection('LANDSAT/LC09/C02/T1_L2')
  .filterBounds(savarGeom)
  .filterDate(START_DATE, END_DATE)
  .filter(ee.Filter.calendarRange(MONTH_START, MONTH_END, 'month'));

// Merge (8 + 9)
var l89raw = l8raw.merge(l9raw);
...
print('L8 scenes (raw):', l8raw.size());
print('L9 scenes (raw):', l9raw.size());
print('L8+9 scenes (raw merged):', l89raw.size());

print('L8+9 scenes retained (QC >= ' + SCENE_VALID_FRAC_MIN + ' validFrac):', l89.size());
print('Retained validFrac list:', l89.aggregate_array('validFrac'));

// Apply flood filter BEFORE compositing
l89 = l89.map(addWaterFracPerScene_L8L9)
         .filter(ee.Filter.lte('waterFrac', MAX_WATER_FRAC));

print('Scenes retained after flood filter:', l89.size());
print('Retained waterFrac list:', l89.aggregate_array('waterFrac'));


// -------------------------
// 8) Thermal composite: p75 LST (°C) — PRIMARY LST surface
// -------------------------
var lstCol = l89.select('LST_C');
...
// Use the SAME retained scenes
var srCol = l89.map(addIndices_L8L9);
...
var EVER_WATER_THR = 20; // keep fixed across years
